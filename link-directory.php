...
function wplinkdir_importexport($Type='import'){
	...
	$getPorts=mysql_query("SELECT * FROM ".($Type=='import' ? WPLD_LINKS_TABLE : $wpdb->prefix."links"));
	$Target=get_option('wplinkdir_target');
	$Rel=get_option('wplinkdir_target');

	foreach($getPorts as $Port){
		if($Type='import'){
			// if($Port['flagged']<5){ $Port['flagged']=5-$Port['flagged']; }else{ 
			...
			mysql_query("INSERT INTO ".($Type=='import' ? $wpdb->prefix."links" : WPLD_LINKS_TABLE)." VALUES ('','{$Port['link_owner']}','{$Port['link_email']}','{$Port['link_name']}','{$Port['link_url']}','{$Port['link_description']}','','{$Port['link_category']}','{$Port['pr']}','0','{$Port['link_updated']}','{$Port['link_updated']}','{$Port['link_rating']}','0','{$Port['link_pending']}','{$Port['link_mailed']}','0','0','0'");
		}else{
			$Port['target']=$Target;
			$Port['rel']=$Rel;
			...
			if($Port['flagged']==0){
				$Port['flagged']==0;
			}elseif($Port['flagged']>4){
				$Port['flagged']=0;
			}else{
				$Port['flagged']=5-$Port['flagged'];
			}

			$getCategory=mysql_query("SELECT * FROM ".WPLD_CATS_TABLE." WHERE id = );
			$getOwner=mysql_query("SELECT * FROM '' WHERE id = );
			mysql_query("INSERT INTO ".($Type=='import' ? $wpdb->prefix."links" : WPLD_LINKS_TABLE)." VALUES ('','{$Port['url']}','{$Port['title']}','','{$Port['target']}','{$Port['category']}','{$Port['description']}','{$Port['pending']}','{$Port['name']}','{$Port['flagged']}','{$Port['date_added']}','{$Port['rel']}','',''");
		}
	}
}
...
function wplinkdir_init(){
	...
	if(($Version!='' && $Version!='1.8') || @mysql_result($getTable,0,0)){
		...
		add_option('wplinkdir_captcha','Yes');
...
function wplinkdir_menu(){
	...
	add_menu_page($WPLD_Trans['WPLD'], $WPLD_Trans['WPLD'].($NewLinks>0 ? "<sup style=\"color:red;font-weight:bold;\">{$NewLinks}</sup>" : ''), 8, __FILE__, 'wplinkdir_admin_page');
	add_submenu_page(__FILE__, $WPLD_Trans['WPLD'].' - '.$WPLD_Trans['WPLD_EditLinks'],$WPLD_Trans['WPLD_EditLinks'], 8, 'wplinkdir_editlinks_page', 'wplinkdir_editlinks_page'); 
	if(WPLD_PRO){
...
function wplinkdir_admin_page(){
	...
	if(isset($_POST['wplinkdir_cleansedelete'])){
	...
	<tr valign="top"><td><b>'.__('Require Captcha?',$WPLD_Domain).'</b></td>';

	unset($Yes,$No);
	if(get_option('wplinkdir_captcha')=='No'){
		$No=' SELECTED';
	}else{
		$Yes=' SELECTED';
	}

	echo '<td><select name="wplinkdir_captcha"><option'.$No.'>'.$_No.'</option><option'.$Yes.'>'.$_Yes.'</option></select></td>
...
function wplinkdir_functions(){
	...
	echo '<h2>'.__('Import Links',$WPLD_Domain).'</h2>
...
function wplinkdir_editlinks_page(){
	...
	if($_POST['Submit']==$WPLD_Trans['Update']){
		...
		if(get_option('wplinkdir_emailthem')=='Yes' && $Email=mysql_result($getMailed,0,0)!='' && $Mailed==0){
			$LinkOut=wplinkdir_linktosite($_POST['url'],$_POST['title'],$_POST['id']);
			$BlogName=get_option('blogname');
			$SiteURL=get_option('siteurl');
			if($wp_rewrite->using_permalinks()==true && $Permalinks!=''){
				$LinkPage=get_bloginfo('url').'/'.get_option('wplinkdir_permalinks').'/';
			}else{
				$LinkPage=preg_replace('/(\&|\?)act\=(addsite|linkus|search)/','',$_SERVER['REQUEST_URI']);
			}
			$Subject=sprintf(__('Your Link Approved At %s',$WPLD_Domain),$BlogName);
			mail($Email,$Subject,sprintf(__('This is just a quick message to let you know that your link has been approved at %1$s and is now live. You can view your link here: '.$LinkOut.'.'."\n\n".'Link Name: %2$s (PR %3$s)'."\n".'Link URL: %4$s'."\n\nNote that your email is not available to the public and that this is an automated message, so please do not reply to it.",$WPLD_Domain),$BlogName,$title,$Rank,$url));
			$Mailed=1;
		}
	...
	while($Link=mysql_fetch_assoc($getLinks)){
		...
		echo '</td><td>Flags: '.$Link['flagged'].'<br>Status: <select name="pending"><option value="0"'.$Approved.'>Approved</option><option value="1"'.$Pending.'>Pending</option></select></td>
...
function wplinkdir_category_page(){
	...
	if(@mysql_num_rows($getCats)>0){
		...
		while($Cat=mysql_fetch_assoc($getCats)){
			...
			<tr valign="top"><td><b>Title:</b><br><input type="text" name="title" value="'.$Cat['title'].'"></td>
...
function wplinkdir_addsite(){
	...
	if($_POST['entry_url']!=''){
		...
		if(get_option('wplinkdir_captcha')=='Yes'){
			...
			if (!$captchas->validate ($random_string)){
				...
				return __('You entered the wrong captcha code. Aren\'t you human? Please use back button and reload.',$WPLD_Domain);
			}
		}
